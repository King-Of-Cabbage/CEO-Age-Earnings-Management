import argparse
from pathlib import Path
from src.config import read_config
from src.load_data import load_project_data
from src.variables import CONTROLS, add_age_bands
from src.model_spec import ModelSpec
from src.estimators import fit_panel, format_result
from src.result_writer import write_rows


def run(config, covariance="cluster_firm", layer="reproduced_primary"):
    cfg = read_config(config)
    df = add_age_bands(load_project_data(cfg["data_path"]))
    rows = []
    for dependent in ["AEM_w", "REM_w"]:
        spec = ModelSpec("Fixed age bands", dependent, ("MidAge", "OldAge"), tuple(CONTROLS), covariance=covariance)
        result, data = fit_panel(df, spec)
        for xvar in spec.xvars:
            rows.append(format_result(spec, result, data, xvar, layer))
    write_rows(Path(cfg["output_dir"]) / f"age_bands_{covariance}.csv", rows)
    return rows


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", required=True)
    parser.add_argument("--covariance", default="cluster_firm")
    args = parser.parse_args()
    run(args.config, args.covariance)
