...
GROUPS = [(1, "Big Four"), (2, "Non-Big Four")]
...
def run(config, covariance="cluster_firm", layer="reproduced_primary"):
    ...
    for dependent in ["AEM_w", "REM"]:
        for value, label in GROUPS:
            spec = ModelSpec("Auditor groups", dependent, ("CEO_age_w",), tuple(CONTROLS), sample=label, covariance=covariance, sample_rule=lambda d, v=value: d["BIGFour"] == v)
            result, data = fit_panel(df, spec)
            rows.append(format_result(spec, result, data, "CEO_age_w", layer))
    write_rows(Path(cfg["output_dir"]) / f"auditor_groups_{covariance}.csv", rows)
    return rows
